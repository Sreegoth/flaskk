...
def profilePage():
    ...
    for i in range(len(images), 12):
        images.append(None)
    return render_template('profile_page.html', name=str(user[0]+" "+user[1]), email=str(email)[2:-2], img1=openImg(images[0]), img2=openImg(images[1]),img3=openImg(images[2]), img4=openImg(images[3]),img5=openImg(images[4]), img6=openImg(images[5]), img7=openImg(images[6]),img8=openImg(images[7]), img9=openImg(images[8]))
